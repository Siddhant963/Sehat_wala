...
module.exports.AddCoustomer = async (req, res) => {
  ...
  try {
    ...
    const mealsTimingArray = meals_timeing
      .split(",")
      .map((time) => time.trim());
...
module.exports.AddSubscription = async (req, res) => {
  ...
  if (subscription) {
    ...
    return res
      .status(200)
      .json({ message: "Subscription added successfully", data: subscription });
  }
};
  ...
  try {
    ...
    if (attendance) {
      ...
      if (attendance.status !== status) {
        ...
        return res
          .status(400)
          .json({ message: "Attendance already marked with same status" });
      }
    } else {
...
module.exports.getallcoustomers = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Coustomers fetched successfully", data: coustomers });
  } catch (error) {
    return res.status(500).json({ message: "Internal server error" });
  }
};
...
module.exports.createDeliveriesByMealType = async (req, res) => {
  try {
    ...
    if (!meal_type || !delivery_date) {
      return res
        .status(400)
        .json({ message: "Meal type and delivery date are required" });
    }
    ...
    const existingCustomerIds = existingDeliveries.map((d) =>
    ...
    if (customersNeedingDelivery.length === 0) {
      return res.status(200).json({
        message:
          "All customers already have deliveries for this date and meal type",
        data: [],
      });
    }
...
module.exports.getallDeliveries = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Deliveries fetched successfully", data: deliveries });
  } catch (error) {
    return res.status(500).json({ message: "Internal server error" });
  }
};
  ...
  try {
    ...
    if (delivery_person_id) {
      if (!mongoose.Types.ObjectId.isValid(delivery_person_id)) {
        return res
          .status(400)
          .json({ message: "Invalid delivery person ID format" });
      }
    ...
    if (status) {
      ...
      if (!validStatuses.includes(status.toLowerCase())) {
        return res.status(400).json({
          message:
            "Invalid status. Valid values are: pending, delivered, cancelled, assigned",
        });
      }
...
module.exports.updateDelivery = async (req, res) => {
  ...
  if (!delivery_id || !customer_id || !status) {
    return res
      .status(400)
      .json({ message: "delivery_id, customer_id, and status are required" });
  }
  ...
  try {
    ...
    if (
      ...
      if (subscription.meals > 0) {
        ...
        return res
          .status(400)
          .json({ message: "No meals left in subscription" });
      }
    }
...
module.exports.updateCoustomer = async (req, res) => {
  ...
  try {
    ...
    if (subscription_start_date)
      coustomer.subscription_start_date = subscription_start_date;
    if (subscription_end_date)
      coustomer.subscription_end_date = subscription_end_date;
    if (meals) coustomer.meals = meals;
    //convert meals_timing to array if it is a string and normalize it
    if (meals_timing) {
      let Cmeals = [];

      if (typeof meals_timing === "string") {
        Cmeals = meals_timing
          .split(/[\s,]+/) // split by space(s) or comma(s)
          .map((time) => time.toLowerCase().trim())
          .filter((time) => time.length > 0); // remove empty entries
      } else if (Array.isArray(meals_timing)) {
        Cmeals = meals_timing.map((time) => time.toLowerCase().trim());
      }
      coustomer.meals_timeing = Cmeals; // store cleaned array
    }
    ...
    return res
      .status(200)
      .json({ message: "Coustomer updated successfully", data: coustomer });
  } catch (error) {
...
module.exports.rechargesubscription = async (req, res) => {
  ...
  if (!coustomer_id || !subscription_id) {
    return res
      .status(400)
      .json({ message: "Coustomer ID and Subscription ID are required" });
  }
  ...
  try {
    ...
    coustomer.subscription_end_date = new Date(
    ...
    return res
      .status(200)
      .json({ message: "Subscription recharged successfully" });
  } catch (error) {
...
module.exports.getallusersbyfillter = async (req, res) => {
  const { _id, name, email, password, contact, total_salary, roles } =
    req.query;
  if (
    !_id &&
    !name &&
...
module.exports.deshbordDetails = async (req, res) => {
  try {
    ...
    const penddingDeliveries = await Deliverymodel.countDocuments({
      status: "pending",
    });
